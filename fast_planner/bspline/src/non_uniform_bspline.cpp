...
void NonUniformBspline::setUniformBspline(const Eigen::MatrixXd& points, const int& order,
                                          const double& interval) {
  //设置控制点control_points_，阶数p_，时间间隔interval_，n_，m_和节点向量u_
  control_points_ = points;
  p_              = order;       //B-spline次数
  interval_       = interval;

  n_ = points.rows() - 1;        //控制点Q:Q0,Q1,...,Qn
  m_ = n_ + p_ + 1;              //节点数量,节点u：u0,u1,...,un+p+1

  u_ = Eigen::VectorXd::Zero(m_ + 1);
  for (int i = 0; i <= m_; ++i) {  

    if (i <= p_) {                           //要确保0时刻Up = 0,因为B-spline的定义域为[Up,Um-p)，故前p个节点时间为负值。Up为规划轨迹的起始时刻。
      u_(i) = double(-p_ + i) * interval_;
    } else if (i > p_ && i <= m_ - p_) {     //中间n-p个节点，Up+1=ts，Up+2=2*ts
      u_(i) = u_(i - 1) + interval_;
    } else if (i > m_ - p_) {                //后面p+1个节点
      u_(i) = u_(i - 1) + interval_;
    }
  }
}
...
Eigen::VectorXd NonUniformBspline::evaluateDeBoor(const double& u) {
  // 先将u定位到某个区间内，再使用递归的deBoor算法求出u时刻B-spline曲线的三维坐标

  double ub = min(max(u_(p_), u), u_(m_ - p_));    //B-spline的定义域为[Up,Um-p)
  ...
  for (int i = 0; i <= p_; ++i) {                   //[Ui,Ui+1)上的轨迹取决于p+1个控制点Qi-p,...Qi
...
Eigen::MatrixXd NonUniformBspline::getDerivativeControlPoints() {
  ...
  // B样条的一阶微分是次数-1，控制点数-1的B样条曲线，因此相应的Knot vector-2
...
NonUniformBspline NonUniformBspline::getDerivative() {
  //调用getDerivativeControlPoints()得到B-spline的导数的控制点后，新初始化一个均匀B样条，阶数为原阶数-1
  //时间间隔一样，区间间隔为原间隔去头去尾，最终得到B-spline的导数曲线。
  Eigen::MatrixXd   ctp = getDerivativeControlPoints(); //在利用公式获得B样条的一阶微分控制点
  NonUniformBspline derivative(ctp, p_ - 1, interval_); //新定义一个NonUniformBspline对象，并将新的控制点，次数，Knot vector赋值给它
...
bool NonUniformBspline::checkFeasibility(bool show) {
  //验证轨迹的速度和加速度是否超过动力学限制（只检查控制点处大小），符合限制返回true，否则返回false,并得到超过最大限制的比例ratio
  bool fea = true;
  ...
  for (int i = 0; i < P.rows() - 2; ++i) {
    ...
    if (fabs(acc(0)) > limit_acc_ + 1e-4 || fabs(acc(1)) > limit_acc_ + 1e-4 ||
      ...
      if (i == 1 || i == 2) {  //对于起点附近的控制点, 强制延长时间的区间为U2 ~ U5(区间更长了), 为了让起始的速度, 加速度不那么激进?
        ...
        for (int j = i + 3; j <= i + p_ + 1; ++j) {      //将增量加到 Ui+3 ~ Ui+p+1 上
...
void NonUniformBspline::parameterizeToBspline(const double& ts, const vector<Eigen::Vector3d>& point_set,
  ...
  //使用最小二乘拟合的方法将一段轨迹点拟合成均匀B样条函数，得到控制点
  if (ts <= 0) {
  ...
  if (start_end_derivative.size() != 4) {      //起始和结尾处的速度以及加速度
    cout << "[B-spline]:derivatives error." << endl;
  }

  int K = point_set.size();   //前端搜索获得的离散轨迹点个数
