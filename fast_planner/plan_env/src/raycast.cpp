...
// 计算下一个经过的体素位置，并存储在 ray_pt中。函数返回值为布尔型，表示是否还有更多的体素需要计算。
// 对于每个体素，通过比较与下一个 X/Y/Z 方向交点的距离来决定下一个沿着哪个轴前进，并调整 t 值（即关于交点位置的距离）。
// 如果进入下一个体素后，当前的位置已经是终点 endX_, endY_, endZ_，则返回 false 表示已经到达终点，计算结束。
