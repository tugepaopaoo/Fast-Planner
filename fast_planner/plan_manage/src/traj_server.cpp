...
void drawCmd(const Eigen::Vector3d& pos, const Eigen::Vector3d& vec, const int& id,
  ...
  mk_state.scale.x = 0.1;    //箭头轴径
  mk_state.scale.y = 0.2;    //箭头头径
  mk_state.scale.z = 0.3;    //箭头头部长度
  ...
  pt.y = pos(1);
  pt.z = pos(2);
  mk_state.points.push_back(pt);  //箭头尾点坐标

  pt.x = pos(0) + vec(0);
  pt.y = pos(1) + vec(1);
  pt.z = pos(2) + vec(2);
  mk_state.points.push_back(pt);  //箭头头点坐标
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  // 根据t_cur计算当前的位置、速度、加速度、偏航角、偏航角速度，即对规划的轨迹进行采样输出
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber odom_sub = node.subscribe("/odom_world", 50, odomCallbck);    // 接收无人机的实际位置信息

  cmd_vis_pub = node.advertise<visualization_msgs::Marker>("planning/position_cmd_vis", 10); // 无人机前的大箭头
  pos_cmd_pub = node.advertise<quadrotor_msgs::PositionCommand>("/position_cmd", 50);        // 发布控制指令
  traj_pub = node.advertise<visualization_msgs::Marker>("planning/travel_traj", 10);         // 期望轨迹

  ros::Timer cmd_timer = node.createTimer(ros::Duration(0.01), cmdCallback);   // 100Hz 发布控制指令，并绘制指令（无人机前的大箭头）
  ros::Timer vis_timer = node.createTimer(ros::Duration(0.25), visCallback);   // 4Hz 绘制期望轨迹
