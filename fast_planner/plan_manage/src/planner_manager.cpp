...
void FastPlannerManager::planYaw(const Eigen::Vector3d& start_yaw) {
  ...
  double dt_yaw  = 0.3;                          //获取航向角yaw的时间增量300ms
  int    seg_num = ceil(duration / dt_yaw);      //计算轨迹分段数，轨迹分段数 = 该轨迹的总运行时间/时间增量
  dt_yaw         = duration / seg_num;           //设置航向角yaw的时间增量，时间增量 = 该轨迹的总运行时间/轨迹分段数
  ...
  // seg_num -> seg_num - 1 点用于约束，不包括边界状态

  //（2）计算路径点waypoints的航向角yaw
  for (int i = 0; i < seg_num; ++i) {                     //遍历所有轨迹分段
    double          tc = i * dt_yaw;                      //迭代计算轨迹现在的时间时刻  
    ...
    if (pd.norm() > 1e-6) {                 //当前控制点与下一段控制点的有向向量达到阈值，就计算yaw
      waypt(0) = atan2(pd(1), pd(0));       //计算向量的夹角，即航向yaw
      waypt(1) = waypt(2) = 0.0;
      calcNextYaw(last_yaw, waypt(0));      //计算下一个路径点的航向
    } else {
      waypt = waypts.back();
    }
