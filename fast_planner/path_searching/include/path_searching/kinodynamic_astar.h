...
class NodeHashTable {
 private:
  /* data */
  std::unordered_map<Eigen::Vector3i, PathNodePtr, matrix_hash<Eigen::Vector3i>>     //static
      data_3d_;
  std::unordered_map<Eigen::Vector4i, PathNodePtr, matrix_hash<Eigen::Vector4i>>     //dynamic
      data_4d_;
 ...
 public:
  ...
  PathNodePtr find(Eigen::Vector3i idx) {
    ...
    return iter == data_3d_.end() ? NULL : iter->second;        //若在hash table中找不到此节点，则返回NULL
  }
